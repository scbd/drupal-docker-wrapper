...
This repo builds one thing: a reusable Drupal 11 base image (`scbd/drupal-docker-wrapper`) that the
bioland system runs as its CMS container. It layers pinned contrib modules, Drush, and CLI tooling
on `drupal:11.x-php8.4`, plus a two-phase startup that gets Apache serving immediately and defers
heavy provisioning to a background script.
...
bioland-head Nuxt frontend consumes the JSON:API this image exposes. See `docs/prd.md` for product
framing and `docs/CONTEXT.md` for vocabulary.

The guarantee is structural, not corrective: the deployed stack bind-mounts exactly five paths from
EFS per site, and `modules/custom` is the only module path among them (see Deployment for the full
contract). Contrib, core, and vendor always come from the pinned image and cannot drift, so no
repair phase re-asserts the pin. Build-time pinning and the runtime mount contract speak one
language about what is pinned versus overlaid, so this is a single bounded context.
...
> Mermaid C4 is experimental. Without C4 support, read this as: the wrapper sits between the
> upstream Drupal image and Composer sources it consumes, the CI and developers who build it, and
> the EFS-backed Swarm runtime plus Nuxt frontend that depend on it. The custom modules and
> bioland-head are named conceptually; their code lives in other bioland repos.

## 3. Containers (C4 L2)

The image is one deployable artifact; this splits what is built in from what is overlaid.
...
The hard rule: only `modules/custom`, `sites`, `drush`, `temp`, and the PHP ini are safe to mount.
Mounting the whole `modules` directory is a volume mask (see `docs/CONTEXT.md`) and defeats the pin.
Never mount `vendor/`, `web/core/`, or `modules/contrib/`.

## 4. Key Components (C4 L3)

### 4.1 Build stages

Three stages, each adding one concern. Keeping module installs in their own stage means a module
...
Two deliberate surprises:

- The `composer-patches` plugin is required in its own step **before** the module
  `composer require`, because patches must be wired before any patched package is pulled.
- `base-core` ignores three guzzle/psr7 advisories (`PKSA-93qv-9n9h-6k6p`, `PKSA-k22t-f949-t9g6`,
  `PKSA-7qs6-zvnz-h66r`), a temporary BL-695 measure so the Critical Drupal core fix in 11.3.12 can
  build before patched guzzle/psr7 releases exist in core's pinned ranges. Tracks Drupal issue
  #3599842; meant to be removed.
...
`apply_patches_if_present` before Apache starts, applying `.patch` files under `patches/` with
`git apply` - no `patch(1)` fallback - and skipping anything already applied, detected with a
`git apply --reverse --check` dry run rather than a marker file. A missing `lib/patches.sh` is
logged and skipped rather than treated as fatal, and a failing patch step logs and continues rather
...
No application database; the "data" is the dependency manifest the build produces. The entities
below are build artifacts, not rows.
...
`composer.lock` is a build-time source of truth only, pinning every contrib module's exact version
at build. Nothing reads it at runtime: only `modules/custom` is ever bind-mounted, so contrib always
comes from the image and has no drift to compare against.
...
repo). Each site's stack bind-mounts exactly five EFS paths: `php/custom.ini`, `modules/custom`,
`sites`, `drush`, `temp`. The whole `modules` directory is never mounted, so `web/modules/contrib`,
`web/core`, and `vendor` always come from the pinned image.
...
- **Startup patch application runs unconditionally when present.** `apply_patches_if_present` runs
  every start if `lib/patches.sh` shipped in the image, before Apache serves. It is idempotent (an
  already-applied patch is detected by a reverse dry-run and skipped) and a failure only logs and
  continues, but there is no way to disable it short of removing `lib/patches.sh` from the image.
- **Temporary advisory ignores.** Three guzzle/psr7 advisories are suppressed for BL-695 and must
  be removed once patched releases land in core's ranges (Drupal #3599842). Left in, they hide real
  future advisories on those packages.
- **Release publishing is off.** The `push-images` job is commented out, so tagged releases build
...
- **No cache rebuild after a deploy.** A deployment shipping new module or patch code must run a
  ...
  container or route table. See `docs/adr/0007-...`.
- **No in-container guarantee on any `web/sites` permission.** After-start does not touch the
  ...
  `sites/*/files/.htaccess`. A mount shipping `settings.php` world-readable stays that way, and the
